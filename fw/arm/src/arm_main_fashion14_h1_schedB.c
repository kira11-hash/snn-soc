/*
 * ARM-side Fashion-14x14 2-layer FC runner for H1 Schedule B board validation.
 * Schedule B keeps baseline thresholds (16, 8) and switches layer 1 to hard reset.
 */
...
#define H1_SCHEDB_LAYER0_THRESHOLD 16u
#define H1_SCHEDB_LAYER1_THRESHOLD 8u
#define H1_SOFT_RESET 0u
#define H1_HARD_RESET 1u
...
static int run_sample(const v2b_fashion_sample_t *sample, uint32_t idx)
{
    int rc = v2b_infer_resident_14x14_h1(
        ...
        H1_SCHEDB_LAYER0_THRESHOLD, H1_SOFT_RESET,
        H1_SCHEDB_LAYER1_THRESHOLD, H1_HARD_RESET,
        counts_buf
    );
...
int arm_main(void)
{
    ...
    uart_init();
    uart_puts("\nUART_OK\n");
    uart_puts("[TB] v2b_arm_fashion14_h1_schedB start - 10 Fashion 14x14 samples via AXI-Lite\n");
