...
（K0 表为 n=2000 口径；下 K0b 为全测试集分布）

### K0b. 输入跳零 cycle 分布（best + honest 双版本，全测试集，2026-06-06）
> Codex 采纳项 + 数据双版本原则。输入跳零只依赖图像、不依赖权重 → 全测试集直接统计（脚本 `/tmp/feas4.py`）。**★ 关键:即使 worst-case 也省 37–67%（不会出现跳零反变慢）→ 有界确定性延迟,是卖点。**
| 数据集 | dense | best(最稀疏) | median | mean | p95 | **worst(最坏)** |
|---|---|---|---|---|---|---|
| Fashion | 25088 | 1208 (省95%) | 6496 (74%) | 6632 (74%) | 10824 (57%) | **15784 (省37%)** |
| KMNIST | 25088 | 712 (省97%) | 4464 (82%) | 4636 (82%) | 7568 (70%) | **13416 (省47%)** |
| MNIST | 25088 | 776 (省97%) | 3240 (87%) | 3320 (87%) | 5240 (79%) | **8272 (省67%)** |
- 论文报法:**典型(mean) + worst-case 都给**;best 仅作"最稀疏样本"上界、不当代表值。MNIST 全集 mean 3320 与 Codex 独立数一致。
