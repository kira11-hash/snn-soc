...
| `clk_div_sel` | 内部半周期计数上限 `div_limit` | 近似分频 | `sys_clk=50MHz` 时 SPI 频率 |
|---|---:|---:|---:|
| 0 | 0 | ÷2 | 25 MHz |
| 1 | 1 | ÷4 | 12.5 MHz |
| 2 | 3 | ÷8 | 6.25 MHz |
...
| 4 | 15 | ÷32 | 1.5625 MHz |
| 5 | 31 | ÷64 | 0.78125 MHz |
| 6 | 63 | ÷128 | 0.390625 MHz |
| 7 | 127 | ÷256 | 0.1953125 MHz |

安全策略（已在 RTL 落地）：
- 当软件写 `CTRL` 时若 `spi_en=1` 且 `clk_div_sel=0`，硬件会自动钳位到 `clk_div_sel=2`（6.25MHz）。
- 目的是避免 bring-up 阶段误配到 25MHz，提升外部 Flash 与板级连线容差。

建议 bring-up 默认用中低速（例如 6.25MHz 或更低），先保功能稳定。
...
- 读回应被钳位为 `0x0000_0005`（`clk_div=2`，6.25MHz @ 50MHz）
