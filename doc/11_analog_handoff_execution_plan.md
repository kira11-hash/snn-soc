...
- `PROG_CTRL.RETRY_LIMIT`：**RTL 复位默认 4**（`snn_soc_pkg.sv:221` `PROG_VERIFY_RETRY_MAX = 4`），字段宽度 3-bit 故上限 7：✅ 够用
...
| A8-8 Retry 次数确认（2026-04-24 器件老师 OK；2026-04-25 口径修正） | **`PROG_CTRL.RETRY_LIMIT` 已确认够用**：RTL 复位默认 4（`rtl/top/snn_soc_pkg.sv:221` `PROG_VERIFY_RETRY_MAX = 4`），3-bit 字段上限 7。**2026-04-25 修正**：早期文档误写为"默认 3 / pkg=7"——pkg 参数 4 是上电默认 reset 值，7 是字段宽度上限（不是 pkg 参数），SW 可在运行时通过 `PROG_CTRL[10:8]` 改写到 0–7。 |
