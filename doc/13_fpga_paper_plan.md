...
本文件中部分“当前状态”段落写于 `SPI` 和 `E203` 接入之前。当前真实状态已经更新为：

- `spi_ctrl` 已替换 `spi_stub`
- `E203` 已完成最小接入
- `bootloader / SPI 启动 / UART printf` 已在仿真中跑通

因此，本文件中凡是写着“SPI 仍停留在 stub / E203 尚未完成”的段落，应视为旧阶段记录，不再作为当前主线判断依据。
