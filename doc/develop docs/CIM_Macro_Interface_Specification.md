...
> **以下内容已更新至 2026-02-27 版本，与 RTL 代码一致。**
...
| dac_valid | 输入 | 1 | 单拍触发；行为模型在该拍锁存 wl_spike（真实芯片由 wl_latch 时序控制） |
...
## 2. 时序流程
1. 数字侧发出 `dac_valid` 单拍，同时 `wl_spike` 保持稳定。
2. 行为模型在 `dac_valid` 单拍锁存 `wl_spike`（无 `dac_ready`）。
3. 数字侧按固定 `DAC_LATENCY_CYCLES` 延迟后发出 `cim_start`，宏在固定延迟后给出 `cim_done`。
