...
- **Iter 10（CIM 编程接口）**：已完成 — `cim_program_ctrl` + `cim_macro_arbiter`，支持 RRAM 阵列写入/擦除/验证
- **Iter 11（多层 SNN 推理）**：已完成 — `layer_sequencer` + `spike_feedback` + `lif_neuron_alu`，`MULTILAYER_SMOKE_PASS` 通过
- **Iter 12（#1/#3 rate coding + neuron plugin）**：已取消
- **Iter 14（#5 CPU-ANN bridge）**：已取消
- **Iter 15（最终集成 + 深度风险审查）**：已完成 — 5 个 RTL 设计风险修复 + 文档全量对齐
...
- V2 RTL（CIM 编程 + 多层推理）已验证通过，不影响 V1 主线
...
| 日期 | 里程碑 | 检查点 | 状态 |
|:---:|:---|:---|:---:|
| 2月13日 | M1: Phase 1完成 | 理解MVP代码和架构 | 已完成 |
| 3月6日 | M2: Phase 2完成 | UART/SPI 实现并验证 | 已完成 |
| 3月27日 | M3: Phase 3完成 | DMA扩展，权重Buffer | 已完成 |
| 4月17日 | M4: Phase 4完成 | AXI-Lite总线升级 | 已完成 |
| **5月15日** | **M5: Code Freeze** | **E203集成，代码冻结** | 已完成（提前） |
| **6月30日** | **M6: Tapeout** | **GDSII提交** | 进行中 |
...
> 以下为 2026-01-30 编写的初始阶段计划，保留作为历史参考。Phase 1-5 已于 2026-03-20 全部提前完成。
> 主体中未勾选的 `[ ]` checkbox 大多已实际完成，不代表当前进度。
...
- Phase 1-5 所有功能全部完成（Iteration 1-7），Phase 4 全量外设集成已通过独立 TB 与主链回归
- Iteration 8（2026-03-24）全量回归基线刷新，13 项仿真 + chip_top 编译/lint + 脚本语法门禁全部通过
- 2026-03-31 全面交叉审计（RTL / TB / sim scripts / 文档），参数一致性 100%，寄存器地址 100% 正确，无硬件阻塞级 bug
- E203 冻结决策已明确

#### 2026-03-21 Status Sync

- UART / SPI / DMA（多目标）/ AXI-Lite bridge 已完成主线接入并通过独立 TB 与主链回归（Iteration 1-4，2026-03-18）
- E203 已完成最小面积接入（Iteration 5，2026-03-18）
- bootloader / SPI 启动 / UART printf 已在专用 E203 Icarus 仿真链路中跑通（Iteration 6，2026-03-19）
- JTAG rescue loader 已完成（Iteration 7，2026-03-20）
- chip_top 已完成 tapeout-intent 信号直连
- 全量仿真回归均通过
...
*初始版本：2026-01-30 / 最后更新：2026-04-18*
