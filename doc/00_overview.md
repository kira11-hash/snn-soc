...
**当前分支状态**：`feature/spi` 已独立实现 `spi_ctrl.sv` + `spi_flash_model.sv`，并通过 `sim/run_spi_icarus.sh` 专项验证；`snn_soc_top` 仍保留 `spi_stub` 作为主线实例。
...
- periph：主线 SoC 仍使用 UART/SPI/JTAG stub；本分支额外提供独立 `spi_ctrl` IP  
...
- 分支新增 IP：`spi_ctrl.sv` + `spi_flash_model.sv` 已通过独立 smoke test，等待接入 `snn_soc_top`。
