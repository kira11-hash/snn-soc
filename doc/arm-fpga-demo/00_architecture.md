...
- **CONV 扩展 + LeNet-5 28×28 板验（feature/v2-arm-fpga-demo-conv）—✅ PASS**（artifact provenance 见 `build_manifest_v2.txt`；原生 conv1 root-cause fix commit = `48958da0`；最新 re-verify PASS marker = `ARM_FPGA_DEMO_LENET5_PASS`，详见 §12）
...
**当前分支状态**：round 4 已补 `tile_partial_buf clear_busy` 握手化 +
cross-shell / manifest hardening，并在 2026-05-04 重新做 fresh UART capture。
**LeNet-5 artifact provenance**：以 `doc/arm-fpga-demo/build_manifest_v2.txt`
中的 bit/XSA/ELF SHA256 为准。
**原生 conv1 root-cause fix commit**：`48958da0`（历史关键修复点，不再是当前 HEAD）
**板验 PASS 标记**：`ARM_FPGA_DEMO_LENET5_PASS`
**对应 manifest**：`doc/arm-fpga-demo/build_manifest_v2.txt`（记录当前 artifact hash）
**最新 UART capture**：`doc/arm-fpga-demo/uart_capture_20260503_round4_r404_reverify.txt`
...
| 维度 | 当前板验状态（artifact provenance 见 `build_manifest_v2.txt`；原生 conv1 root-cause fix = `48958da0`） |
|------|---------------------------------------------|
...
| UART 抓 LeNet-5 PASS marker | ✅ `ARM_FPGA_DEMO_LENET5_PASS`（2026-05-04 round 4 / R404 re-verify，capture 见 `uart_capture_20260503_round4_r404_reverify.txt`） |
| Manifest 文件本体 | ✅ `build_manifest_v2.txt` 已刷新为稳定口径（相对路径、无时间戳），bit/XSA/ELF 以当前 SHA256 为准 |
| native conv1 路径板验事实 | ✅ 当前 artifact set 重新抓到 10/10 `ARM_FPGA_DEMO_LENET5_PASS` |
| 板验日志 | ✅ 历史长日志 `board_bringup_log_lenet5.txt` + 最新 raw capture `uart_capture_20260503_round4_r404_reverify.txt` |
...
- **论文 / 简历**：引用 `build_manifest_v2.txt` + `uart_capture_20260503_round4_r404_reverify.txt`；bit/XSA/ELF provenance 以 manifest 当前 hash 为准
