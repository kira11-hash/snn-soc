...
### VS Code Remote-SSH 与 Codex/Claude Code 的代理配置

使用 **VS Code Remote-SSH** 连接服务器时，AI 工具（Codex/Claude Code 等）的网络请求由 VS Code Server 进程发起，而不是终端 shell——因此仅靠 `proxy_on` 函数对 **VS Code Server 进程无效**。

需要把代理环境变量写入 VS Code Server 的启动环境文件：
...
修改后需要重启 VS Code Server 使配置生效：
1. 在 VS Code 命令面板执行 `Remote-SSH: Kill VS Code Server on Host...`
2. 重新连接服务器

修改后验证是否生效：在 VS Code 内置终端执行 `curl -I https://www.google.com`，返回 HTTP 200/301 即表示代理正常。

> **注意**：`curl -I https://api.openai.com` 可能返回指向 `auth.openai.com` 的 Cloudflare challenge 或 403，这不代表代理不通——通常是服务端 IP 封锁或 API Key 鉴权问题，与代理本身是否工作无关。
