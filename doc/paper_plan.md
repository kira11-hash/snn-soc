...
| E0.5 配置同步与 smoke 重基线 | 让后续全部实验建立在 E0 冻结后的统一口径上 | 1. 按 E0 结果确认主前端方法选择（硬件参数 ADC=8/W=4/T=3/ratio_code=4 大概率不变，`snn_soc_pkg.sv` 只需确认一致）；2. 用 E0 选出的主前端方法，通过 [export_weights.py](D:/SoC%20Design/SoC%20Design/fpga/scripts/export_weights.py) 重新导出 `weight_pos.hex/weight_neg.hex`；3. 重新导出匹配新前端预处理的测试输入 `test_image.hex`；4. 更新文档中的主前端方法名称（如从 `proj_sup_64` 改为 `avgpool_8x8`）；5. 重新跑数字仿真和 smoke test（Icarus light），确认主链通过；6. 只有 smoke 全通过，A-F 的正式结果才开始记录；7. 旧 Step2/Step3 目录不再作为正式结果来源 | [snn_soc_pkg.sv](D:/SoC%20Design/SoC%20Design/rtl/top/snn_soc_pkg.sv)、[export_weights.py](D:/SoC%20Design/SoC%20Design/fpga/scripts/export_weights.py)、E0 输出 `results/paper_e0/stage2/summary.json`、现有 `sim/` 和 `tb/` 流程 | 一份 post-E0 frozen config 记录；新的 `weight_pos.hex/weight_neg.hex/test_image.hex`；重新通过的 smoke test 结果 | 不单独成图；作为 A-F 的前置门槛 |
