...
| 6. Design-Point Locking | 参数为什么这样选；Python 在文中扮演什么角色；在固定 64 输入接口下，非神经网络前端主配置如何确定；内部定量 baseline 如何定义 | Fig.5 参数锁定图；Table.4 最终配置表 | [run_all.py](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/run_all.py)、[summary.txt](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/results/summary.txt)、[exhaustive_64dim_schemeB_full.csv](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/results/exhaustive_64dim_schemeB_full.csv) 以及本轮 `avgpool_8x8` 起点的 shortlist 重跑结果；最终数值以 [snn_soc_pkg.sv](D:/SoC%20Design/SoC%20Design/rtl/top/snn_soc_pkg.sv) 为准 |
| 7. Validation and Prototype Evidence | 你的主实验 A/B/C/D 是什么，证据够不够；内部验证 baseline 是否闭环 | Table.5 协议/时序一致性；Fig.6 Python vs L2；Fig.7 Test Mode；Table.6 FPGA 实现结果 | 见第 3 节实验计划；实现报告来自 [project_2 impl_1](D:/SoC%20Design/SoC%20Design/project_2/project_2.runs/impl_1) 和 [project_5 impl_1](D:/SoC%20Design/SoC%20Design/project_5/project_5.runs/impl_1) |
| 8. Discussion and Limitations | 你没有真实宏/流片数据，边界在哪 | Table.7 限制与未来工作 | 直接基于实验边界与接口契约表 |
| 9. Conclusion | 你解决了什么，没解决什么 | 无新图表 | 全文总结 |

**3. 完整实验计划**

| 实验 | 目的 | 具体操作步骤 | 输入/脚本 | 预期输出 | 论文对应 |
|---|---|---|---|---|---|
| E0 参数冻结核对 | 消灭口径冲突，尤其是 `summary.txt` 与 `pkg` 不一致，并在固定 64 输入接口下确定非神经网络主配置与内部定量 baseline | 1. 以 [snn_soc_pkg.sv](D:/SoC%20Design/SoC%20Design/rtl/top/snn_soc_pkg.sv) 冻结最终硬件参数；2. 从 `avgpool_8x8` 开始重跑与论文决策直接相关的 shortlist：`avgpool_8x8`、`pad32_zero_8x8`、`pad32_reflect_8x8`、`proj_sup_64`，可选补跑 `pad32_replicate_8x8`、`maxpool_8x8`；3. 优先复核 `avgpool_8x8` 的历史高点配置，至少覆盖 `ratio=0.30`；4. 不重跑 `7x7` 系列、`proj_pca_64` 和全量无关方法；5. 改了 preprocessing / method 时不要使用 `--skip-train`；6. 用 [summary.txt](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/results/summary.txt) 和 `csv` 只做 rationale；7. 写一页 `paper_facts.md` 记录最终口径，并把 `proj_sup_64` 降级为 learned upper-bound reference；8. 将 `avgpool_8x8`、`pad32_zero_8x8`、`pad32_reflect_8x8` 形成主文内部定量 baseline，`proj_sup_64` 仅作为 learned upper-bound reference，不作为主部署前端 | [summary.txt](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/results/summary.txt)、[exhaustive_64dim_schemeB_full.csv](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/results/exhaustive_64dim_schemeB_full.csv)、[snn_soc_pkg.sv](D:/SoC%20Design/SoC%20Design/rtl/top/snn_soc_pkg.sv)、[config.py](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/config.py)、[run_all.py](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/run_all.py) | Table.4 最终配置表；Fig.5 参数锁定图 | Section 6 |
| A 协议/时序一致性 | 证明 `control-path invariance under macro substitution` | 1. 不新增 TB，直接复用 [top_tb_icarus_light.sv](D:/SoC%20Design/SoC%20Design/tb/top_tb_icarus_light.sv)；2. L0：当前分支打开 `REG_CIM_TEST`；3. L1：在 blackbox 路径下运行；4. L2：在 FPGA model 路径下运行；5. 三种模型各自生成 VCD；6. 用脚本提取 `u_cim_ctrl.state`、`u_adc.state`、`cim_start_pulse`、`cim_done`、`adc_start`、`adc_done` 的变化时刻；7. 对比阶段顺序与阶段 cycle 数 | 复用 [top_tb_icarus_light.sv](D:/SoC%20Design/SoC%20Design/tb/top_tb_icarus_light.sv)、[run_icarus_light.sh](D:/SoC%20Design/SoC%20Design/sim/run_icarus_light.sh)、[cim_macro_blackbox.sv](D:/SoC%20Design/SoC%20Design/rtl/snn/cim_macro_blackbox.sv)、[cim_fpga_model.sv](D:/SoC%20Design/SoC%20Design/fpga/cim_model/cim_fpga_model.sv)；新增的仅是一个 VCD/log 提取脚本，建议放在 `tools/` 或 `fpga/scripts/` | 三种模型的状态转移序列相同；阶段 cycle 数在相同 delay 参数下相同 | Table.5，Fig.6 左半 |
| B L2 vs Python 结果等价性 | 证明相同权重下，L2 路径与 Python golden 的输出一致 | 1. 用 [export_weights.py](D:/SoC%20Design/SoC%20Design/fpga/scripts/export_weights.py) 导出 `weight_pos.hex/weight_neg.hex`；2. 额外导出 `test_image.hex`；3. 新增 `tb/tb_python_equiv.sv` 读入该图像；4. **Python golden 必须直接加载 `weight_pos.hex / weight_neg.hex` 的整数值作为量化权重，而不是从 float 权重重新量化**；5. 若仓库中没有现成反向加载能力，则新增一个简单的 `hex -> numpy` 加载脚本，建议放在 `fpga/scripts/`；6. Python 侧保存同一图像或小 batch 的 `spike count vector`；7. L2 路径跑同一输入；8. 比较 10 维输出向量或 `OUT_FIFO_COUNT` | [run_all.py](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/run_all.py)、[export_weights.py](D:/SoC%20Design/SoC%20Design/fpga/scripts/export_weights.py)、[run_cim_fpga_test.sh](D:/SoC%20Design/SoC%20Design/fpga/sim/run_cim_fpga_test.sh) 作为 L2 单元测试模板；输入必须包含 `weight_pos.hex`、`weight_neg.hex`、`test_image.hex` | Python vs L2 的结果对照表；至少 1 图 + 1 表 | Fig.6 右半，Table.6 |
| C CIM Test Mode bring-up | 证明无宏条件下数字链可自检 | 1. 用 `REG_CIM_TEST` 设置 4 组注入：`100/0`、`60/20`、`20/20`、`0/100`；2. 记录 `OUT_FIFO_COUNT`、`dbg_*` 计数、`adc_sat`；3. 写明当前是统一 `pos/neg` 注入，不能逐 neuron 分类；4. 不做过度 claim | [reg_bank.sv](D:/SoC%20Design/SoC%20Design/rtl/reg/reg_bank.sv)、[snn_soc_top.sv](D:/SoC%20Design/SoC%20Design/rtl/top/snn_soc_top.sv)、[top_tb_icarus_light.sv](D:/SoC%20Design/SoC%20Design/tb/top_tb_icarus_light.sv) | 一张单调性/场景矩阵图；一段 limitation 说明 | Fig.7，Table.7 |
| D FPGA 实现证据 | 提供 Python/VCS 给不了的 implementation evidence | 1. 从 Step2 提取 `WNS/LUT/FF/BRAM/power/DRC`；2. 从 Step3 提取 `xsa/bit/hwh` 和 implementation 报告；3. 统一成一张 implementation table；4. 功耗标注为 FPGA estimate | Step2 归档见 [doc/14_fpga_fullversion_execution.md](D:/SoC%20Design/SoC%20Design/doc/14_fpga_fullversion_execution.md) 和 [project_2 impl_1](D:/SoC%20Design/SoC%20Design/project_2/project_2.runs/impl_1)；Step3 见 [project_5](D:/SoC%20Design/SoC%20Design/project_5) 与 [project_5 impl_1](D:/SoC%20Design/SoC%20Design/project_5/project_5.runs/impl_1) | Table.8 FPGA implementation summary | Section 7 |
| E ARM P0 最小闭环 | 为 main-text 提供最小板级系统证据 | 1. 下载 [bd_step3_arm_wrapper.xsa](D:/SoC%20Design/SoC%20Design/project_5/bd_step3_arm_wrapper.xsa) 和 `bit`；2. ARM 侧只做最小闭环：读寄存器、写输入、启动、读回 `OUT_FIFO_COUNT`；3. 不追求先做 batch | [top_fpga_arm.sv](D:/SoC%20Design/SoC%20Design/fpga/boards/zcu102/top_fpga_arm.sv)、[doc/14_fpga_fullversion_execution.md](D:/SoC%20Design/SoC%20Design/doc/14_fpga_fullversion_execution.md) | 一张最小闭环截图/表格；若通则写入主文，若不通则降级 | Section 7 主文或附录 |
| F 系统完整性补充 | 证明这不是孤立核，而是 complete digital SNN SoC with RISC-V control plane | 1. UART/SPI 合入后复跑现有 unit test；2. E203 合入后做最小固件 smoke：配置寄存器、启动一次推理、读回结果；3. 只作 system completeness，不作为 novelty | [run_uart_icarus.sh](D:/SoC%20Design/SoC%20Design/sim/run_uart_icarus.sh)、[run_spi_icarus.sh](D:/SoC%20Design/SoC%20Design/sim/run_spi_icarus.sh)、E203 合入后的顶层/固件路径 | 一张 system completeness checklist | 附录或 Section 3 最后一小节 |
...
| Fig.5 | 参数锁定图：只放 `8x8 非神经网络前端 shortlist 重跑对比`、`ADC sweep`、`Scheme A/B` 三个最小子图，并明确前者是主文内部定量 baseline | [Python建模 results](D:/SoC%20Design/SoC%20Design/项目相关文件/器件对齐/Python建模/results) 中本轮 `avgpool_8x8` 起点的 shortlist 重跑输出，以及既有 `fig2/fig4/fig6` | 3 合 1 拼图 |
...
| Week 2 | 复用 [top_tb_icarus_light.sv](D:/SoC%20Design/SoC%20Design/tb/top_tb_icarus_light.sv) 和现有 Icarus 流程，完成 Experiment A 的 L0/L1 仿真；新增 VCD/log 提取脚本；继续完成 shortlist 重跑并整理接口契约表；起草 Section 3/4/5 | Table.3 接口契约表、Experiment A 原始日志、Fig.5/ Table.4 初版 |
| Week 3 | 完成 Experiment B、C、D；优先攻 ARM P0：寄存器读写/写输入/启动/读回 `OUT_FIFO_COUNT`；E203/UART/SPI 按流片主线推进 | Fig.6/7 初版、FPGA implementation table、ARM P0 结果 |
| Week 4 | 写全文初稿；把 E203/UART/SPI 的系统完整性证据写成一小节或附录；统一图表风格 | Draft v1 |
...
ARM fallback：
- 若 Week 3 末 ARM 仍不通，论文主文删除“board-level batch validation”表述
- 保留 `Experiment A/B/C + Step2/Step3 implementation evidence`
...
| 质疑 | 回应策略 | 需要额外数据 |
|---|---|---|
| 1. 没有真实 CIM 宏/流片数据，为什么能发 | 本文聚焦数字侧与预硅验证连续性，不声称真实宏数值行为；用接口契约表、Experiment A/C、Step2/3 实现证据支撑 | Table.3、Experiment A/C、Table.8 |
...
执行优先级一句话总结：
先做 `Week 1 的 frozen facts + Related Work + avgpool_8x8 起点的 shortlist 重跑`，随后只盯四件事：`Experiment A`、`Experiment B`、`Experiment C`、`Experiment D`；ARM 只要求打通 `P0 最小闭环`，不要一开始把自己拖进 batch 测试泥潭。