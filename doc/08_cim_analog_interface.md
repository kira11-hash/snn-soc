...
- 内部并行口径（当前 `snn_soc_top` / `cim_macro_blackbox`）：
  `wl_spike[63:0] + dac_valid/ready + cim_start/done + bl_sel[4:0] + adc_start/done + bl_data[7:0]`
...
3. 复用发送完成后，进入内部 `dac_valid/ready` 与后续 `cim_start` 流程。  
...
- 模拟侧负责：
  接收 `wl_data/wl_group_sel/wl_latch` 并完成组锁存；CIM/ADC 模拟实现与 `cim_done/adc_done/bl_data` 返回时序。  
- 后端/集成负责：
...
| `dac_valid` | D→A | 1 | 握手 | DAC 数据有效，保持到 ready |
| `dac_ready` | A→D | 1 | 握手 | DAC 准备好接收，为1时锁存数据 |
| **CIM 计算接口** |||||
...
- ADC 使用 8-bit：设备测量确认 8-bit 精度已足够（建模验证 91.24% 测试精度）
### 2.2 信号详细说明
...
**时序要求**：
- 在 `dac_valid` 拉高期间必须保持稳定
- 在 `dac_valid && dac_ready` 时刻被锁存
...
**约束**：
- `dac_valid` 拉高后，在 `dac_ready` 之前不能撤销
- `dac_ready` 可以在 `dac_valid` 之前或同时拉高
- 握手完成（valid && ready 同时为 1）后，下一拍数据被锁存
...
|:---|---:|:---:|:---|
| 时钟周期 | 20 | ns | 50MHz |
| `dac_valid` 保持时间 | ≥1 | cycle | 直到 ready |
...
| DAC 建立时间 | 5 cycles | ✓ | `dac_ready` 延迟 |
...
# 输入从模拟部分（模拟→数字）
set_input_delay 3.0 -clock clk [get_ports {dac_ready cim_done adc_done bl_data*}]
```
...
| 测试项 | 描述 | 负责方 |
|:---|:---|:---|
| DAC 握手 | valid/ready 协议正确性 | 数字+模拟 |
...
module cim_macro_blackbox #(
    ...
    input  logic        dac_valid,              // DAC 数据有效
    output logic        dac_ready,              // DAC 准备好
