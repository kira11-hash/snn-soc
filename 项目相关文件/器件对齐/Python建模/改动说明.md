...
### 4.5 硬件对齐口径修复（重要）
- 新增 **Hardware-aligned（spike-only, no fallback）** 指标：
  - 禁用“零脉冲样本自动回退到 membrane 决策”的兜底逻辑
  - 用于和当前 RTL（只输出 spike 事件，不输出 membrane 兜底路径）保持一致
- `run_all.py` 在 final 报告阶段会同时记录：
  - 常规 `spike` 口径准确率（可能含 zero-spike fallback）
  - **硬件对齐口径** `spike-only` 准确率
  - `zero_spike_rate` / `zero_spike_count`（零脉冲样本占比）
- `summary.txt` 会明确打印上述两套口径，避免“Python 看起来更高、硬件落地变低”的误解
- 注意：本次修复优先解决“评估口径透明性”，**不改变现有全量扫描/推荐逻辑**；若要按硬件对齐口径重新选最终参数，需要重跑 `run_all.py`
