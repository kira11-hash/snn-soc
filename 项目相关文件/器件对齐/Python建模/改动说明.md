...
### 4.6 定版锁定与阈值码值确认（2026-02-27）
为把 Python 结果直接落地到 RTL 默认参数，本轮新增了“定版锁定”流程：

1) **阈值候选一并输出（run_all.py）**
- 在 `calibrate_threshold_ratio(...)` 增加候选结果表输出；
- summary 会记录每个候选的 `ratio -> val_acc`；
- 目的：不只看“被选中的 ratio”，而是完整比较 `2/255, 3/255, 4/255` 的差异，便于固化到寄存器码值。

2) **候选码值试验配置（config.py）**
- 将候选固定为：`THRESHOLD_RATIO_CANDIDATES = [2/255, 3/255, 4/255]`；
- 只保留最终方法与固定扫描项：`proj_sup_64 + ADC=8 + W=4 + T=10`；
- 目的：去掉无关组合，缩短确认时间，避免干扰。

3) **锁定运行模式（config.py）**
- 最终锁定为：
  - `SPIKE_THRESHOLD_RATIO = 4/255`
  - `CALIBRATE_THRESHOLD_RATIO = False`
  - `TUNE_SPLIT = "val"`，`FINAL_REPORT_SPLIT = "test"`
- 目的：进入“固定参数复现”模式，防止再次调参导致口径漂移。

4) **本轮确认结论（server final lock）**
- 最终配置：`proj_sup_64, Scheme B, ADC=8, W=4, T=10, ratio_code=4 (4/255)`；
- Final test（spike-only）：`90.42%`，`zero-spike=0.00%`；
- 结论：可直接用于 RTL 默认值定版（`THRESHOLD_RATIO_DEFAULT=4`，`TIMESTEPS_DEFAULT=10`）。

### 4.7 定版后可读性补丁（2026-02-27）
为避免 `4/255=0.015686...` 在日志中被四舍五入显示成 `0.02` 引起误判，新增了比率显示格式化：

1) **run_all.py 新增 `format_ratio()`**
- 统一把 ratio 以高精度显示；
- 若 ratio 正好是 8-bit 码值（`n/255`），同时显示为 `0.xxxxx (n/255)`。

2) **所有关键打印/summary 输出改用 `format_ratio()`**
- 包括：
  - threshold calibration 输出；
  - method 对比输出；
  - full-grid best-case/recommendation 输出；
  - summary.txt 中的 best-case / recommendation / top-k / calibration 段。

3) **改动目的**
- 防止把 `3/255`、`4/255` 这类接近值误看成同一个 `0.02`；
- 让 Python 报告与 RTL 寄存器码值一一对应，便于直接定版与复核。
