...
SPIKE_THRESHOLD_RATIO = 0.01  # 固定阈值比例（用于当前 ADC 位宽专项扫描）
...
# 调参阶段请保持 "val"；参数冻结后再手动改回 "test" 跑最终一次报告
FINAL_REPORT_SPLIT = "val"
...
# 当前关闭标定，固定使用 SPIKE_THRESHOLD_RATIO=0.01。
# 保留候选仅用于后续重新开启标定时的默认列表。
THRESHOLD_RATIO_CANDIDATES = [0.01]
...
# 鍙傛暟鎵弿鑼冨洿
# =====================================================
# ADC 位宽专项扫描（固定 ratio/W/T）
ADC_BITS_SWEEP    = [6, 8, 10]
WEIGHT_BITS_SWEEP = [4]
TIMESTEPS_SWEEP   = [10]
