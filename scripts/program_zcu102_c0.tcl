...
# Vivado 2022.2 emits psu_init.tcl under .gen/sources_1/bd/.../ip/..._zynq_ultra_ps_e_0_0/;
# older flows put it at .srcs/sources_1/bd/.../hw_handoff/. Search both plus any
# fallback location under the project dir.
if {$psu_init_override ne ""} {
  ...
  set _candidates [list \
    ...
    "$root_dir/fpga_synth/zcu102_arm_demo/**/psu_init.tcl" \
  ]
...
puts "\[program_zcu102_c0\] resolved psu_init.tcl: $psu_init"
...
# psu_init must run with PSU target, not A53 — mask_write goes via JTAG-DAP APB
# path; targeting A53 (APU Reset) makes it try CPU accesses which fail.
targets -set -filter {name =~ "PSU"}
after 200
psu_init
after 1000
...
# Now reset just the A53 core and download ELF
...
puts "(Optional explicit psu_init override: xsct scripts/program_zcu102_c0.tcl /abs/path/to/psu_init.tcl)"
