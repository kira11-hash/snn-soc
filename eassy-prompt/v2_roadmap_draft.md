# V2 路线图（DC 之前） — 第 4 稿（数据集口径改为 MNIST + 可配 FC 架构卖点）

> 本稿在第 3 稿基础上根据用户决定把数据集主线从 Fashion-MNIST 改回 MNIST，
> 论文 B 的核心卖点改为"**可配置多层 FC 网络加速器**"（层数可配 + 每层 BL 数可配），
> MNIST 作为 demo 而非挑战数据集。Fashion-MNIST 降级为可选 stretch。

**基线日期**：2026-04-18
**分支**：`v2`
**目标**：DC 综合前完成全部数字功能验证 + FPGA 端到端 + 可配多拓扑 demo
...
|---|---|---|---|
| ~~A0~~ | ✅ **完成** | — | 9/9 绿 |
| A1 | 新建 `python_multilayer/`，复用 V1 框架 | 3 天 | MNIST 加载 + 4 层网络骨架 OK |
