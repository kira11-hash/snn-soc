...
### 9.9 ★★ 读出延迟 1-10μs 的重大影响（老师 2026-06-06）+ 待澄清 + 优化重定向
老师给读出时间 **1-10μs/次**。时钟 50MHz=20ns → **一次读 = 50-500 个时钟周期**，读比数字算慢 2-3 个数量级 → 若属实，芯片延迟/能耗几乎全由"读次数"决定，数字 fmax 无关紧要。
- **★ 关键待澄清（决定读是否瓶颈，需再问老师）**：1-10μs 是**模拟 ADC 精确读**的时间，还是**我们 1-bit 数字 sense（判 LRS/HRS）也这么久**？1-bit sense（单比较器、不精确量化）通常远快于多比特 ADC；但小电流（10nA-0.1μA）建立慢对两者都有。问："1-bit 判 0/1 要多久？"
  - **情形 A**（数字 sense 快 ~ns-100ns）→ 读非瓶颈，原 cycle 模型成立，且**"数字读快 vs 模拟慢"= 大卖点**。
  - **情形 B**（数字也 1-10μs）→ 读是绝对瓶颈，延迟≈读次数×1-10μs（Fashion ~6632 次→**6.6-66ms**），**不能吹超低延迟**，卖点转能效/鲁棒。
- **优化重定向（两情形都对，稳健）**：① 跳零（减读次数）= 头号，读越慢越值；② **一次读尽量多（宽读口/列广播）从 §9.8 future 升级为高价值**（读固定开销大，一次读整行 984 vs 128 省 ~8× 读次数）；③ **读-算流水（算藏进读）**：读慢则必做（算 20ns 藏进读 μs）；④ §B popcount-free：**读是瓶颈则 fmax 提速无意义 → 选最省面积版**；读快则 fmax 有意义。
- **counter 必加**：read_count(hidden/output) + 实测读时间 → 论文延迟/能效核心。
- **不阻塞 RTL 开工**（功能 bit-exact 不依赖读时间/能耗）；阻塞的是延迟/能效 **headline 定性**（待"数字 sense 时间"澄清 + 能耗数）。
