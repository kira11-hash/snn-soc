...
//   T10: 未映射地址写返回 DECERR
//   T11: 未映射地址读返回 DECERR + 零数据
//   T12: 已映射但未对齐写返回 DECERR，且无副作用
//   T13: 已映射但未对齐读返回 DECERR + 零数据
//
// 【通过标准】
...
module axi_bridge_tb;
  ...
  initial begin
    ...
    axi_write(TB_ADDR_REG_BASE + 32'h10, 32'h0000_0BF4, 4'hF);  // 0x0BF4=3060：本 TB 仅做寄存器读写桩验证，写入值为任意常数，与当前默认 T=10/threshold=2550 无关
    ...
    if (rd_data === 32'h0000_0BF4) begin
      ...
      $display("[FAIL] T3 reg[4] : got=0x%08X exp=0x00000BF4", rd_data);
      fail_cnt = fail_cnt + 1;
    end
    ...
    // ── T12/T13: 已映射但未对齐地址同样返回 DECERR，且不透传到下游 ──────────
    fail_mark = fail_cnt;
    axi_write_check_resp(TB_ADDR_REG_BASE + 32'h01, 32'h1122_3344, 4'h1, 2'b11);
    axi_read(TB_ADDR_REG_BASE + 32'h00, rd_data);
    if ((fail_cnt == fail_mark) && (rd_data === 32'hDEAD_BEEF)) begin
      $display("[PASS] T12 unaligned write DECERR + no side effect");
      pass_cnt = pass_cnt + 1;
    end else begin
      $display("[FAIL] T12 unaligned write side effect: got=0x%08X exp=0xDEADBEEF", rd_data);
      fail_cnt = fail_cnt + 1;
    end

    fail_mark = fail_cnt;
    axi_read_check_resp(TB_ADDR_REG_BASE + 32'h02, 2'b11, rd_data);
    if ((fail_cnt == fail_mark) && (rd_data === 32'h0000_0000)) begin
      $display("[PASS] T13 unaligned read DECERR + zero data");
      pass_cnt = pass_cnt + 1;
    end else begin
      $display("[FAIL] T13 unaligned read data: got=0x%08X exp=0x00000000", rd_data);
      fail_cnt = fail_cnt + 1;
    end
