...
//   T5  cmd_addr 在事务期间"粘住"：RD_WAIT 拍 cmd_addr 仍等于原值，
//       不会回到 default（这是 adapter 存在的原因）
//   T6  连续多笔 write+read，验证 FSM 能稳定循环
//
...
module simple2v2btop_adapter_tb;
  ...
  // read_mux 对直接寄存器组（非 SBA/SBB）N 拍就 ready；这里我们 mock
  // 的 rsp_rdata = {20'h0, cmd_addr}，即地址本身做签名。
  ...
  initial begin
    ...
    // ── T5: cmd_addr 在事务期间稳定 ─────────────────────────────────
    $display("[T5] cmd_addr stays stable throughout transaction");
    do_read_check_addr_stable(32'hA000_0F00, rd);
    if (rd !== 32'h0000_0F00) begin
      $display("[ERR] T5 rdata mismatch: rd=0x%08h", rd);
      errors++;
    end

    // ── T6: 连续多笔 write + read ───────────────────────────────────
    $display("[T6] back-to-back mixed traffic");
    do_write(32'hA000_0010, 32'h1111_0001, d, 8);
    if (d !== 2) begin $display("[ERR] T6.w1 delay=%0d", d); errors++; end
    do_read (32'hA000_0010, rd, d, 8);
    if (d !== 3) begin $display("[ERR] T6.r1 delay=%0d", d); errors++; end
