...
module top_tb;
  ...
  // -----------------------------------------------------------------------
  // Pad-side IO tie-offs for unused external interfaces in this TB.
  logic uart_rx;
  logic uart_tx;
  logic spi_cs_n;
  logic spi_sck;
  ...
  // -----------------------------------------------------------------------
  // DUT 实例化：snn_soc_top
  // 外部 IO 在这里被拉到安全值，实际激励通过内部 bus_simple_if 进行。
  // -----------------------------------------------------------------------
  snn_soc_top dut (
    .clk      (clk),
    .rst_n    (rst_n),
  ...
  //   - 同时将所有 pad-side 输入置为安全状态，避免 X 传播
