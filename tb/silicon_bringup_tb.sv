...
module silicon_bringup_tb;
  ...
  // Fast firmware-regression path: run silicon_bringup directly from
  // instr_sram@0x0. The slower boot_rom / 0x1000 handoff is covered by the
  // dedicated chip_top_rom_smoke regressions, so this TB focuses on the
  // firmware's digital self-test logic and program/infer sequencing.
  snn_soc_top #(
    .ENABLE_E203(1'b1),
  ...
  integer poll;
  initial begin
    // R401: default regression must not spend most wall-clock time dumping a
    // full-chip VCD. Keep waveform capture opt-in for local debug only.
