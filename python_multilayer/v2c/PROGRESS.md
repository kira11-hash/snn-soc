...
- **⚠ Phase B analog 基线 — 建模问题已修，结论：锚定文献、家酿仅 illustrative**：home-grown 模拟 CIM 模型 ① 旧 ADC 全局 full-scale=max|sum| 被离群值主导 → Fashion σ=0 崩 69.65%(数据集相关假象)；**已改 per-output full-scale → σ=0 修回 ≈clean（Fashion 87.35 / MNIST 97.35 / KMNIST 89.30）**。② **但 per-weight 电导 σ 在大 fan-in MAC 里被平均掉 → σ 效应太轻**(Fashion σ0→0.3 仅 −1.5pp、MNIST −0.65pp、KMNIST −2.5pp)，**不构成"模拟崩"**。根因：真实模拟脆弱主要来自**不被平均的项(drift/ADC 动态范围/IR-drop/非线性/read noise)**，纯 per-weight-σ 低估之(故此模型对模拟是 **optimistic**)。**最终方案：① V2C 数字侧用实测优雅降级曲线；② "模拟崩"锚定已发表数据(σ>10% 退化、drift 1月 68%→19%、SAF)；③ 家酿 σ-模型仅作 illustrative/optimistic 补充(明确标注，且其"即使乐观也随 σ 退化、而数字 by-design 零暴露"仍是有效论点)；④ analog 公平性列为 Codex#3 重点审查项。**
