...
- **下一步（用户流程）**:① **出 Codex prompt**(复审 surrogate+多比特输入+所有修复 的正确性/PPA口径;**让 Codex 上论文调研如何闭合 7pp spiking-vs-ANN gap** + 定位) ② 按反馈修调 ③ 之后转**非理想鲁棒性 demo**(注 stuck-at/bit-flip/read-BER,画 accuracy-vs-故障率,对比模拟 CIM 脆弱 = by-design 稳赢)+ 再出 Codex prompt ④ ramp 的 golden `forward.py` 正式实现 / 三数据集 sweep。
