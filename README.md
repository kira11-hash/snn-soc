...
## 分支说明
- 当前 `feature/uart-tx` 分支新增了 `rtl/periph/uart_ctrl.sv` 与 `bash sim/run_uart_icarus.sh`，用于 UART TX 控制器的 IP 级验证。
- `snn_soc_top` 当前仍实例化 `uart_stub`；本分支的 `uart_ctrl` 已通过独立回归，等待后续顶层替换与 SoC 联调。
...
rtl/   RTL 实现
  ...
  periph/   SPI/JTAG stub + UART 控制器原型
...
- 当前 SoC 主线实例仍使用 UART/SPI/JTAG stub；本分支额外提供未接入顶层的 `uart_ctrl.sv` 与独立烟雾测试。
