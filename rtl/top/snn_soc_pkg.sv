...
// 当前默认配置：proj_sup_64 + Scheme B + ADC=8 + W=4 + T=3 + ratio_code=4(4/255)
// 说明：T=10 作为论文高精度对照档位，不作为工程默认值
//======================================================================
package snn_soc_pkg;
  ...
  // 默认 T=3 时最大累积约 = 255 × 2^7 × 3 = 97920 < 2^17，32 位绰绰有余
  // ──────────────────────────────────────────────────────────────────────────
  parameter int LIF_MEM_WIDTH = 32; // 膜电位寄存器位宽（有符号 32-bit）

  // ──────────────────────────────────────────────────────────────────────────
  // 阈值比例寄存器默认值（8-bit 码值，定版锁定）
  // ratio_code=4 → 4/255 ≈ 0.0157（工程默认 T=3；论文可用 T=10 做高精度对照）
  ...
  // 工程默认：T=3（精度/时延折中）；T=10 作为高精度对照点
  parameter int TIMESTEPS_DEFAULT = 3; // 默认推理帧数（工程默认）

  // 阈值默认值：ratio_code × (2^PIXEL_BITS - 1) × TIMESTEPS
  // = 4 × (256-1) × 3 = 4 × 255 × 3 = 3060
  ...
  // 需要覆盖 TB 一次性灌入的 bit-plane 数量（TIMESTEPS * PIXEL_BITS = 3 * 8 = 24）
  // 实际需求 24，选 256 提供充足余量，对仿真面积无影响
  // ──────────────────────────────────────────────────────────────────────────
