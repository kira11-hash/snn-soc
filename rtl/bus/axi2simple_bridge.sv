...
//
// 【地址合法性约束】
//   - 访问地址必须命中 snn_soc_pkg 中定义的地址窗口
//   - 访问地址必须满足 4B 对齐（addr[1:0] == 2'b00）
//   - 任一条件不满足时，不向 bus_simple 发请求，直接返回 DECERR
//
// 【升级路径】
...
module axi2simple_bridge (
  ...
  function automatic logic addr_word_aligned (
    ...
    addr_word_aligned = (addr[1:0] == 2'b00);
  endfunction
  ...
  assign wr_addr_bad = fire_wr && (!addr_mapped(wr_addr_eff) || !addr_word_aligned(wr_addr_eff));
  assign rd_addr_bad = fire_rd && (!addr_mapped(s_araddr) || !addr_word_aligned(s_araddr));

  // ── bus_simple 主机输出（组合逻辑）──────────────────────────────────────
  // m_valid 在请求发起同一拍驱动（cycle N），
  // bus_interconnect 寄存一拍后在 cycle N+1 产生 m_ready/m_rvalid
  assign m_valid = (fire_wr && !wr_addr_bad) || (fire_rd && !rd_addr_bad);
  assign m_write = fire_wr && !wr_addr_bad;
  assign m_addr  = fire_wr ? wr_addr_eff : s_araddr;
  assign m_wdata = wr_data_eff;
  assign m_wstrb = (fire_wr && !wr_addr_bad) ? wr_wstrb_eff : 4'b0;
      ...
      case (state)
        ...
        ST_IDLE: begin
          ...
          if (fire_wr) begin
            ...
            wr_decerr_rsp <= wr_addr_bad;
            if (wr_addr_bad) begin
              state <= ST_WR_RSP;
            end
          end else if (fire_rd) begin
            rd_decerr_rsp <= rd_addr_bad;
            if (rd_addr_bad) begin
              rdata_reg <= 32'h0;
              state <= ST_RD_RSP;
            end else begin
              state <= ST_RD_PEND;
