...
已经做的：1. ✅ **单张图多次呈现**（多帧重复输入）- 通过 TIMESTEPS/THRESHOLD 调节
...
13) **CIM Test Mode + Debug 计数器 + FIFO 断言（2026-02-08）**
    - **CIM Test Mode**：`snn_soc_top.sv` 新增测试模式 MUX + 响应生成器
      - `reg_bank.sv` 新增 `REG_CIM_TEST`（0x2C），bit[0]=test_mode, bits[15:8]=test_data
      - `cim_test_mode=1` 时旁路 `cim_macro_blackbox` 的 4 个输出（dac_ready/cim_done/adc_done/bl_data）
      - 数字侧生成 fake 响应：dac_ready=1, cim_done 延迟 2 拍, adc_done 延迟 1 拍, bl_data=test_data
...
- **引脚预算**（更新后的估算）：
  - 推理模式：wl_spike×64 + dac_valid/ready×2 + cim_start/done×2 + bl_sel×5 + adc_start/done×2 + bl_data×8 + clk + rst_n + mode ≈ **86 pin**
...
- ✅ CIM Test Mode（硅上单测用，避免依赖真实宏）— 已实现（2026-02-08）
...
#### 简化握手协议（省掉 dac_valid/ready/adc_start/adc_done，共省 4 pin）
