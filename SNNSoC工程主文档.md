...
### 截至 2026-03-03 的 FPGA 论文执行补充（不覆盖 V1 流片主线）

1. 论文主结果路径固定为：`ARM（PS）完整量化`。
2. 系统完整性补充路径固定为：`E203 最小演示`。
3. 不在首稿内强制做 E203 全量，原因是工程成熟度、内存/I/O条件和 6.30 截止时间风险。
4. 首稿创新点以低风险可量化项为主：
   - 稀疏感知计算跳过
   - 稀疏统计可解释化
   - Shadow Buffer（可选增强）
   - 性能计数器差分协议 + 中断模式（可观测性创新）
5. 计数器寄存器口径采用：
   - `0x30`: `cim_cycle_cnt[31:16]`, `dma_frame_cnt[15:0]`
   - `0x34`: `wl_stall_cnt[31:16]`, `spike_cnt[15:0]`
6. 中断模式采用增强策略：新增状态/掩码/清除/超时寄存器；异常时可回退轮询。
7. 执行四原则（固定）：
   - 先 baseline（ARM 主路径完整量化 + E203 最小闭环）
   - 再创新 A/B（同一数据集、同一时钟、同一配置）
   - 采样框架/日志脚本可前置（观测工具）
...
|**B. BRAM+流水权重表（升级版）**|将 CIM 存储/计算改成 BRAM 端口化 + 流水 MAC（或等价结构）|时序/扩展性更稳，适合做优化版 A/B 结果|

**截至 2026-03-03 更新**：当前 `fpga-fullversion-snnsoc` 默认实现是 `readmemh` 权重数组 + 组合 MAC（偏 LUT/组合），并非“已完成 BRAM+流水”。
建议执行顺序是“先当前组合基线跑通，再做 BRAM+流水升级版”，这样你能在 FPGA 上验证：
...
1. “建议用方案 B（BRAM 权重表）”这句要改成“V1 bring-up 先当前组合基线，再 BRAM+流水升级版”
