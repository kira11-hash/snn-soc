...
1、Smoke test 是"冒烟测试"（点火后不冒烟就算过），但要保证质量，必须逐模块深入验证。这也是为什么 `doc/06_learning_path.md` 中建议做模块级仿真的原因。

2、**参数口径**：默认参数与时序常量以 `rtl/top/snn_soc_pkg.sv` 为准，本文数值仅作说明，若不一致以 pkg 为准。
...
5、添加模块的时候的标准记得以本文档第三部分为准

6、注意： SPI 片外 Flash “读很久”风险（新增，2026-03-01）（下面有）
## 前言：先完成V1基础功能，再加创新点
...
阶段6a (2周)：挑选低风险创新点逐个添加（64×16映射、活动检测、时钟门控）- 每个创新点都要有对比测试（加前 vs 加后）（这里记得！！！！）
...
- 这一步风险最小、最容易成功
### 与此同时，与数字SoC V1同时进行，其他同学负责：模拟CIM MACRO的设计与流片（阵列采用氧化物RRAM）

单独TAPE OUT: 模拟宏 + RRAM后道集成（氧化物RRAM作为CIM MACRO的阵列）
...
本SoC 里，硬件负责跑数据链路，软件负责“配置、调度、收结果”。
